...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        else if (path.startsWith("/app/more")) setSlider(4);
    }, [location.pathname]);
    return (
        <nav
            className={`flex fixed bottom-3 left-1/2 -translate-x-1/2 rounded-full opacity-100 -translate-y md:-translate-y-full md:opacity-0 transition-all ease-in-out duration-300 justify-around items-center bg-surface dark:bg-gray-800 shadow-large max-h-16 min-h-16`}
        >
            <div
                style={{
                    transform: `translateX(${slider * 100}%)`,
                }}
                className="absolute duration-300 ease-in-out aspect-square rounded-full left-0 h-full w-1/5 bg-primary"
            ></div>
            <Link
                title={"Home"}
                onClick={() => {
